...
public class Keyboard {
  ...
  /**
   * Returns the next key that was typed by the user (that the client has not
   * already processed).
   * This method should be preceded by a call to {@link #hasNextKeyPlayed()} to
   * ensure that there
   * is a next key to process. This method returns a lowercase character
  ...
  public char nextKeyPlayed() {
    synchronized (keyLock) {
      if (keysTyped.isEmpty()) {
        throw new NoSuchElementException("your program has already processed all typed keys");
      }
  ...
  private Font getFont(int defaultFontSize, double width, double height) {
    ...
    if (height < initialHeight / 2.0)
      size = 0;
    if (width < initialWidth / 2.0)
      size = 0;
  ...
  private class Key {
    ...
    private void draw(Graphics2D g, double width, double height, Color backgroundColor,
